...
Don't have Julia handy? [`docs/ThermoExplorer_notebook_printout.pdf`](docs/ThermoExplorer_notebook_printout.pdf)
is a static printout of the notebook — every section, with each
slider/dropdown at its documented default, generated by actually running
the notebook's own code (not hand-recreated). It won't stay in sync with
notebook edits automatically; regenerate it after making changes if you
